...
const workflowTemplateSchema = new mongoose.Schema({
  ...
  nodes: [{
    ...
    type: { 
      type: String, 
      required: true,
      enum: ['start', 'end', 'tool', 'llm', 'condition', 'human-review', 'humanReview', 'api', 'webhook', 'trigger', 'default']
    },
    position: {
    ...
    data: {
      label: String,
      description: String,
      tool: String,           // Tool name from LangChain service
      model: String,          // LLM model for AI nodes
      ...
      outputMapping: Object,  // Output variable mapping
      conditions: Object,     // Conditional logic for routing
      timeout: Number,        // Node timeout in seconds
  ...
  edges: [{
    ...
    type: { 
      type: String, 
      default: 'default',
      enum: ['default', 'conditional', 'error', 'timeout']
    },
    conditions: Object,       // Edge conditions for conditional routing
  ...
  metadata: {
    category: {
      ...
      required: true,
      enum: [
        'customer-service', 'analytics', 'marketing', 'sales', 
        'operations', 'hr', 'finance', 'automation', 'integration',
        'ai-analysis', 'reporting', 'notifications', 'business-intelligence', 'other'
      ]
    },
    tags: [String],
    complexity: {
...
workflowTemplateSchema.pre('save', function(next) {
  ...
  if (this.isModified('nodes') || this.isModified('edges')) {
    ...
    // Extract tools used
    this.metadata.toolsUsed = [...new Set(
      this.nodes
