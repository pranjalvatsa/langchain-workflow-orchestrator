...
class AuthService {
  ...
  async refreshToken(refreshToken) {
    try {
      ...
      const tokenRecord = user.refreshTokens.find((token) => token.token === refreshToken && token.expiresAt > new Date());
  ...
  generateTokens(user) {
    ...
    const refreshToken = jwt.sign({ userId: user._id, type: "refresh" }, this.jwtSecret, { expiresIn: this.refreshTokenExpiresIn });
  ...
  async updateUserProfile(userId, updates) {
    try {
      ...
      const user = await User.findByIdAndUpdate(userId, { $set: sanitizedUpdates }, { new: true });
